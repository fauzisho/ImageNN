...
class ImageClassifier:
        ...
        def load_images_from_folder(folder, label):
            ...
            for filename in os.listdir(folder):
                ...
                if ext in self.image_exts:
                    ...
                    try:
                        # Resize to 44x92 as specified in the paper
                        img = load_img(filepath, color_mode='grayscale', target_size=(44, 92))
    ...
    def build_profile_view_cnn(self):
        """Builds a CNN architecture based on the structure described in the paper."""
        model = Sequential([
            # Stage 1: Convolution + Subsampling
            Conv2D(32, (5, 5), activation='relu', input_shape=(44, 92, 1), padding='same'),
            AveragePooling2D(pool_size=(2, 2), strides=2),

            # Stage 2: Convolution + Subsampling
            ...
            # Flatten and Fully Connected Layer (Classifier)
            Flatten(),
            Dense(1, activation='sigmoid')  # Binary classification (vehicle or background)
        ])
